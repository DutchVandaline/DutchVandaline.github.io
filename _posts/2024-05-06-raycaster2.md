...
`if(cos(degToRad(ra))>0.001)` checks if the player is looking right. If cos value is greater than 'small positive value(0.001)', which means cos is between the angle of $0$∽$\frac{\pi}2$ and $\frac{3\pi}\2$∽$2\pi$.<br>
