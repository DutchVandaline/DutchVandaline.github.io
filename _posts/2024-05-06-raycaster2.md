...
```
<br>
Buttons are for the struct that I've made in the "Raycaster Breakdown 1". OpenGL needs to define each movement. If `a` is pressed it moves spins left, if `d` is pressed it spins right. It's obvious. `pdx` and `pdy` is for rotation.
<br>
